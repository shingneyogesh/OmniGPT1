...
public class HistorySection extends BaseTest {
    ...
    public void executeEditPromptWorkflow() {
        setupReport();
        logger.info("Starting the Edit Prompt Workflow...");

        ExtentTest test = extent.createTest("Click On History Section ", "Test the editing of a prompt with reporting");

        try {
            performEditPrompt(test);
            captureScreenshot(test, "Click On History Sectiont");
            test.pass("Click On History Section test passed successfully.");
        } catch (Exception e) {
    ...
    private void performEditPrompt(ExtentTest test) {
        ...
        try {
            ...
            // Click on the history section element
            WebElement historySection = driver.findElement(By.xpath("//*[@id='root']/div/div[1]/div/div/section/section[3]/div[2]/div[1]"));
            historySection.click();  // Click on the history section

            logger.info("Clicked history section element.");
            test.info("Clicked history section element.");
            Thread.sleep(10000);
